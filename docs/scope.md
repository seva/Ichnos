...
| L2 | CCE MCP server live: first channel served over fast-path (API/protocol) to a sanctioned registered consumer on prompt, in a real session — first live target is **opencode** (Owner ruling 2026-09-15: the OpenClaw-gateway framing was unnecessary drift) | claude-path registration proven (grok-research-mcp live via `claude mcp add-json`); opencode registration proven in operator config (`grok-research`, `memory` in opencode.json); gateway path proven as the **L3 second consumer** (`openclaw mcp set` + hot-reload applied 2026-09-15) — both registration paths exercised (`docs/consumer-registration.md`) |
| L3 | Multi-channel context matrix: per-channel precedence (API fast-path vs OS fallback) with per-channel auth state; second local MCP consumer — **reached 2026-09-16**: github + memory channels live, consumers opencode + openclaw proven | memory channel: mcp-memory service (localhost HTTP, query-driven retrieval); github channel: gh CLI; gemini consumer registered (HTTP surface via Tailscale Funnel — **live**, not pending) |
...
| L5 | Web surfaces consume the same context: browser-based interfaces retrieve operator context on prompt, scope-limited by consumer identity — **first consumer live 2026-09-17**: Gemini web UI via Tailscale Funnel (OAuth 2.1, DCR, capability-scoped memory read/write) | AGA adapter lineage (contract reference — its primary-session design is forbidden by the Host Isolation invariant); the MCP native custom-app path replaced the userscript approach |
...
- **Adoption band:** consumers are MCP clients. First live consumer: **opencode** (claude-compatible path proven with grok-research-mcp; Owner ruling 2026-09-15 — OpenClaw gateway de-scoped as L2 target, retained as dormant registered consumer). Web surfaces arrive via the MCP protocol or the adapter contract referenced from AGA — never its primary-session design.
- **Platform band:** Windows host with WSL2 + Docker present; Hyper-V unverified (requires elevation). The display-isolation spike settles headless-on-host vs WSLg vs Hyper-V vs Docker+VNC against the structural-isolation constraint (headless-first question: an automated browser may need no display at all).
...
- **Composes with current system:** the MCP server extends the grok-research-mcp FastMCP skeleton; opencode consumes it live (`mcp.ichnos-cce`); other MCP consumers (OpenClaw dormant, claude-compatible runners) register through the same pattern.
