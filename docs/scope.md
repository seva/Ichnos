...
| L3 | Multi-channel context matrix: per-channel precedence (API fast-path vs OS fallback) with per-channel auth state; second local MCP consumer — **reached 2026-09-16**: github + memory channels live, consumers opencode + openclaw proven | memory channel: mcp-memory service (localhost HTTP, query-driven retrieval); github channel: gh CLI; gemini consumer registered (HTTP surface; Funnel exposure pending) |
