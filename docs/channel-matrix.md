...
| Telegram (bot-mediated) | messages to/from the bot (VixeYult channel) | Bot API; token present in `openclaw.json` (`channels.telegram.botToken`) | bot token | present ✓ (rate limit: ~30 msg/s per chat, 20/min per group — Bot API docs; not probed live) | **fast-path** |
| Telegram (personal) | the operator's own chats and history | none — Bot API sees only bot-addressed traffic; no user API on this host (structural argument, not a probe) | n/a | walled (structural) | **fallback** (isolated display, HITL for login/2FA) |
| OpenClaw session store | agent session transcripts, memory DBs | local sqlite reads (`~/.openclaw/memory/` — `amara.sqlite` named in store dir; currency unverified, see open item 3) | none (local files) | present ✓ (amara last write 2026-03-27 — predates July gateway upgrade) | **fast-path** (local read) |
...
| Browser profiles (Chrome, Edge) | history, browsing **metadata only** — cookie/session values are NEVER served at fast-path (terminal bound); they are the Phase 3 auth-seeding tier (browser-mediated extraction → display session, `docs/auth-seeding.md`) | profile DBs exist on disk (Cookies DB exclusively locked while the browser runs — copy routes in `docs/auth-seeding.md`); *driving* logged-in sites requires a live session | browser-mediated extraction; credential store only (terminal bound) | profiles present ✓ | **split**: local DB reads (metadata) = fast-path; live site sessions = **fallback** |
| WhatsApp | messaging context | none in this project — integration **deferred as a tentative future decision** (issue #3): candidate direction is a standalone fork of grok-research-mcp; Baileys-class filter-excluded (structural — an argument, not a probe; issue #3) | n/a (deferred) | declared-in-config, disabled ✓ (openclaw.json) | **none** — deferred; reactivates only by Owner ruling on issue #3 |
| Email | operator mail | none verified on this host | UNKNOWN | not verified (see note) | **deferred** — operator to name the account/API before this row compiles |
...
|---|---|---|---|
| GitHub | 300 s | 10 s | high event volume (pushes, CI, issues); REST latency modest |
