...
| Browser profiles (Chrome, Edge) | history, browsing **metadata only** — cookie/session values are NEVER served at fast-path (terminal bound); they are the Phase 3 auth-seeding tier (DPAPI → display session) | profile DBs exist on disk; cookie decryption requires the DPAPI rung; *driving* logged-in sites requires a live session | DPAPI (credential-store only, terminal bound) | profiles present ✓ | **split**: local DB reads (metadata) = fast-path; live site sessions = **fallback** |
