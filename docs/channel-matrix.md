...
| WhatsApp | messaging context | declared in `openclaw.json` (`channels.whatsapp`) but `enabled: false` — not an active channel | n/a (disabled) | present-in-config, disabled ✓ | **none** — recompile if enabled |
