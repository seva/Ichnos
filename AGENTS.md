...
- **Scope position** (`docs/scope.md`, updated by Orient): L3 reached (multi-channel: github + memory live; second consumer openclaw proven); next rung is L4 (fallback tier). The ladder climbs to L15 (galactic civilizational); far rungs are heuristic shapes — Orient measures only against the next rung.
