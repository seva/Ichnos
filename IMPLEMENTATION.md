...
- [ ] `secret-scrub` boundary module — **DEFERRED** (2026-09-16, Owner ruling: the module is not built now). Purpose when built: redact credential-shaped content from message-body payloads. Steward note (advisory, not a gate): message-body channels built before the scrub serve content with an accepted, declared risk — the read-only bound already shields walled-garden reads; the Operator has ruled the sending-side risk out of scope.
- [ ] *(next step)* Telegram-bot adapter (`tests/test_telegram.py` first; token ladder per the matrix) — ungated; the deferred scrub can be built into it later without rework (scrub slots into the Channel boundary)
